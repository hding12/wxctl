...
def decode(raw_type: int, message_content: bytes | str | None, packed_info_data: bytes | None, context) -> DecodeResult:
    ...
    if xml_text is None:
        return DecodeResult(kind="voice", text=None, decoded={}, assets=[])
    ...
    assets = [
        dict(a) for a in resolve_structured_assets(
            ...
            packed_info_data, asset_kind="voice",
        )
    ]
    return DecodeResult(kind="voice", text=None, decoded=attrs(voice), assets=assets)
